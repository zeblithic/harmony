...
# Detect if we're in a worktree (for cleanup at the end).
IS_WORKTREE=false
WORKTREE_PATH=""
...
if [[ "$IS_WORKTREE" == "true" ]]; then
  ...
  # ── Standard: switch to main and pull ─────────────────────────────
  echo ""
...
echo "Main:    $(cd "$MAIN_REPO" && git log --oneline -1)"
echo ""
echo "Task complete. Use /findtask to survey what's next."
if [[ "$IS_WORKTREE" == "true" ]]; then
  echo "NOTE: Run 'cd $MAIN_REPO' — your shell may still be in the deleted worktree."
fi
