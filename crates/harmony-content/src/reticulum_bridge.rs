//! Reticulum content bridge — maps content books to resource transfers.
//!
//! Each book (up to ~1MB) maps 1:1 to a single Reticulum resource transfer.
...
/// CID header length prepended to book data for transport.
const CID_HEADER_LEN: usize = 32;

/// Pack a content book for transport: CID header (32 bytes) + book data.
///
...
/// Unpack received transport data: extract CID header and verify book integrity.
///
/// Returns `(cid, book_data)` if the CID matches the data's hash.
...
pub fn unpack_from_transport(transport_data: &[u8]) -> Result<(ContentId, Vec<u8>), ContentError> {
    ...
    let book_data = &transport_data[CID_HEADER_LEN..];

    // Recompute CID from the book data and verify it matches.
    let computed_cid = ContentId::for_book(book_data, received_cid.flags())?;

    if computed_cid != received_cid {
        return Err(ContentError::ChecksumMismatch);
    }

    Ok((received_cid, book_data.to_vec()))
}
...
mod tests {
    ...
    fn unpack_detects_corrupt_content() {
        ...
        // Corrupt a byte in the book data (after the 32-byte CID header).
