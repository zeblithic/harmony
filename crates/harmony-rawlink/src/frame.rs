...
///   `[6 origin_mac][2 key_len (BE)][key_expr][payload]`
///
...
pub fn encode_data_frame(
    ...
    // Payload (rest of frame, no length prefix)
...
///
/// Returns `(src_mac, origin_mac, key_expr, payload)`. Everything after
/// the key expression is treated as payload.
...
pub fn decode_data_frame(
    ...
    if frame.len() < key_end {
        return Err(RawLinkError::FrameError(format!(
            "data frame too short for key_expr: {} < {key_end}",
            frame.len(),
        )));
    }
