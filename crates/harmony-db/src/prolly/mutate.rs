...
/// use the last child.
fn walk_to_leaf(data_dir: &Path, root: ContentId, key: &[u8]) -> Result<TreePath, DbError> {
    ...
    loop {
        ...
        match node {
            ...
            Node::Branch(entries) => {
                if entries.is_empty() {
                    return Err(DbError::CorruptIndex("empty branch node".into()));
                }
                let child_idx =
                    match entries.binary_search_by(|e| e.boundary_key.as_slice().cmp(key)) {
                        Ok(i) => i,
    ...
    loop {
        ...
        if let Some(parent) = parent {
            ...
            if next_idx < parent.entries.len() {
                let sibling_cid = ContentId::from_bytes(parent.entries[next_idx].child_cid);
                let sibling_entries = load_leaf_entries(data_dir, sibling_cid)?;

                if let Some(first) = sibling_entries.first() {
                    if !current_chunk.is_empty()
                        && config.is_boundary(current_size, first.approx_size(), &first.key)
                    {
...
mod tests {
    ...
    fn walk_to_leaf_finds_correct_leaf() {
        ...
        assert!(
            !path.levels.is_empty(),
            "200-entry tree should have branch levels"
        );
    ...
    fn rechunk_after_insert_produces_valid_leaves() {
        ...
        let mut leaf = path.leaf_entries.clone();
        let ins_idx = leaf
            .binary_search_by(|e| e.key.as_slice().cmp(b"k-000100-new".as_slice()))
            .unwrap_err();
        ...
        assert!(
            !result.new_children.is_empty(),
            "rechunk should produce children"
        );

        // All children should be valid leaf nodes.
        for child in &result.new_children {
            let node =
                Node::read_from_cas(dir.path(), ContentId::from_bytes(child.child_cid)).unwrap();
    ...
    fn incremental_insert_matches_rebuild() {
        ...
        let incremental_root =
            incremental_insert(dir.path(), initial_root, &new_leaf_entry, &config).unwrap();
    ...
    fn incremental_insert_into_empty_tree() {
        ...
        assert!(
            root.is_some(),
            "inserting into empty tree should produce a root"
        );
    ...
    fn incremental_upsert_matches_rebuild() {
        ...
        let incremental_root =
            incremental_insert(dir.path(), initial_root, &updated_leaf, &config).unwrap();
    ...
    fn incremental_remove_matches_rebuild() {
        ...
        let incremental_root =
            incremental_remove(dir.path(), initial_root, &remove_key, &config).unwrap();
    ...
    fn incremental_remove_last_entry() {
        ...
        assert!(
            result.is_none(),
            "removing last entry should produce None root"
        );
    }
    ...
    fn incremental_remove_nonexistent_key() {
        ...
        assert!(
            result.is_err(),
            "removing nonexistent key should return error"
        );
    }
    ...
    fn incremental_update_meta_matches_rebuild() {
        ...
        assert!(
            incremental_root.is_some(),
            "update_meta should return a root"
        );
    ...
    fn incremental_sequence_matches_rebuild() {
        ...
        assert_eq!(
            root, reference,
            "100 sequential incremental inserts must match full rebuild"
        );
    }
    ...
    fn incremental_mixed_ops_matches_rebuild() {
        ...
        if let Some(e) = expected
            .iter_mut()
            .find(|e| e.key == format!("k-{:06}", 25).as_bytes())
        {
            e.metadata.flags = 99;
