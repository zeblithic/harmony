...
fn stolen_master_attacker_device_remains_provisional_when_real_devices_dont_vouch() {
    ...
    state.add_liveness(LivenessCert::sign(&attacker_sk, state.owner_id, 1_500_001).unwrap()).unwrap();
    ...
    state.add_vouching(VouchingCert::sign(&device_a_sk, state.owner_id, attacker_id, Stance::Challenge, 1_500_002).unwrap()).unwrap();
    assert_eq!(
...
fn reclamation_refuted_by_predecessor_liveness() {
    ...
    let predecessor_liveness = LivenessCert::sign(&predecessor_sk, predecessor_owner_id, 2_000_500).unwrap();
