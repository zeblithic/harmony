...
use harmony_owner::{
    ...
    lifecycle::{
        enroll_via_master, evaluate_reclamation, mint_owner, mint_reclaimed, ReclamationStatus,
    },
    pubkey_bundle::{ClassicalKeys, PubKeyBundle},
    trust::{
        evaluate_trust, RefusalReason, TrustDecision, DEFAULT_ACTIVE_WINDOW_SECS,
...
fn stolen_master_attacker_device_remains_provisional_when_real_devices_dont_vouch() {
    ...
    let _device_a_id = *mint.state.enrollments.keys().next().unwrap();
    ...
    .unwrap();
    state
        .add_enrollment(r.enrollment_cert, 1_500_000, DEFAULT_ACTIVE_WINDOW_SECS)
        .unwrap();
    for v in r.auto_vouch_certs {
        state.add_vouching(v).unwrap();
    }
    state
        .add_liveness(LivenessCert::sign(&attacker_sk, state.owner_id, 1_500_001).unwrap())
        .unwrap();
...
fn revoked_device_cannot_be_un_revoked() {
    ...
    // Self-revoke
    let rev = RevocationCert::sign_self(
        &device_a_sk,
        state.owner_id,
        device_a_id,
        1_000_500,
    ...
    let rev2 = RevocationCert::sign_self(
        &device_a_sk,
        state.owner_id,
        device_a_id,
        999_000,
...
fn reclamation_refuted_by_predecessor_liveness() {
    ...
    let _predecessor_device_id = *predecessor_mint.state.enrollments.keys().next().unwrap();
    let predecessor_sk = predecessor_mint.device_signing_key;

    // M2 publishes reclamation
    let reclaim = mint_reclaimed(
        predecessor_owner_id,
        DEFAULT_CHALLENGE_WINDOW_SECS,
        "thought all devices were lost".into(),
        2_000_000,
    )
    .unwrap();

    // Predecessor publishes liveness within window
    let predecessor_liveness =
        LivenessCert::sign(&predecessor_sk, predecessor_owner_id, 2_000_500).unwrap();

    let status = evaluate_reclamation(
        &reclaim.reclamation_cert,
        &[predecessor_liveness],
        2_001_000,
    );
...
fn reclamation_honored_after_silent_window() {
    let reclaim =
        mint_reclaimed([7u8; 16], 1000, "fire took everything".into(), 1_000_000).unwrap();
