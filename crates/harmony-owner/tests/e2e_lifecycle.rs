...
fn full_three_device_lifecycle() {
    ...
    .unwrap();
    state
        .add_enrollment(r1.enrollment_cert, 1_001_000, DEFAULT_ACTIVE_WINDOW_SECS)
        .unwrap();
    ...
    .unwrap();
    state
        .add_enrollment(r2.enrollment_cert, 1_002_000, DEFAULT_ACTIVE_WINDOW_SECS)
        .unwrap();
    ...
    let revocation = RevocationCert::sign_self(
        &device_b_sk,
        state.owner_id,
        device_b_id,
        1_003_000,
    ...
    state
        .add_liveness(LivenessCert::sign(&device_a_sk, state.owner_id, way_later).unwrap())
        .unwrap();
