# Oluo CAS Caller-Side Persistence Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build the caller-side persistence layer that handles OluoEngine's `PersistSnapshot` actions — DAG-ingesting into CAS, managing the local head file, and restoring on startup.

**Architecture:** New `persist` module in harmony-oluo with stateless free functions (`persist_snapshot`, `load_snapshot`). Postcard for manifest serialization in CAS, JSON for the local head file. Mirrors harmony-db's `persist.rs` patterns.

**Tech Stack:** harmony-content (DAG ingest/reassemble, BookStore, MemoryBookStore), postcard, serde_json, hex, tempfile (dev)
...
- Create: `crates/harmony-oluo/src/persist.rs`
- Modify: `crates/harmony-oluo/src/lib.rs`

- [ ] **Step 1: Add dependencies to Cargo.toml**

Add `harmony-content`, `serde_json`, `hex`, and `thiserror` to `[dependencies]` and `tempfile` to `[dev-dependencies]`:
...
- [ ] **Step 2: Create persist.rs with error type and SnapshotManifest**
...
Add `pub mod persist;` and re-exports to `crates/harmony-oluo/src/lib.rs`:
...
Run: `cargo check -p harmony-oluo`
Expected: compiles with no errors

- [ ] **Step 5: Commit**

```bash
git add crates/harmony-oluo/Cargo.toml crates/harmony-oluo/src/persist.rs crates/harmony-oluo/src/lib.rs
git commit -m "feat(oluo): add persist module scaffold with SnapshotManifest and error types (ZEB-110)"
```

---

### Task 2: Implement `persist_snapshot`

**Files:**
- Modify: `crates/harmony-oluo/src/persist.rs`

- [ ] **Step 1: Write the failing test**

Add to the bottom of `persist.rs`:
...
Run: `cargo test -p harmony-oluo persist_snapshot_creates_head_file`
Expected: FAIL — `persist_snapshot` function body doesn't exist yet

- [ ] **Step 3: Implement `persist_snapshot` and helper functions**

Add above the `#[cfg(test)]` block in `persist.rs`:
...
```bash
git add crates/harmony-oluo/src/persist.rs
git commit -m "feat(oluo): implement persist_snapshot with DAG ingest and head file (ZEB-110)"
```
...
Run: `cargo test -p harmony-oluo load_snapshot_returns_none_when_no_head`
Expected: FAIL — `load_snapshot` function doesn't exist

- [ ] **Step 3: Implement `load_snapshot`**

Add after `persist_snapshot` in `persist.rs`:
...
```bash
git add crates/harmony-oluo/src/persist.rs
git commit -m "feat(oluo): implement load_snapshot with DAG reassembly and engine restore (ZEB-110)"
```
...
This test exercises the full flow: create an engine, ingest entries, trigger compaction, persist the snapshot, load it back, and verify the restored engine produces identical search results.

- [ ] **Step 1: Write the round-trip test**

Add to the `tests` module in `persist.rs`:
...
```rust
    ...
    fn ingest_and_compact(engine: &mut OluoEngine, count: usize) -> OluoAction {
        for i in 0..count {
            ...
            let actions = engine.handle(OluoEvent::Ingest {
                ...
                decision: IngestDecision::IndexDurable,
    ...
    fn persist_and_load_round_trip() {
        ...
        // Note: we need a mutable engine to call handle, but load_snapshot
        // returns an owned engine.
...
Run: `cargo test -p harmony-oluo persist_and_load_round_trip`
Expected: PASS
...
```bash
git add crates/harmony-oluo/src/persist.rs
git commit -m "test(oluo): persist/load round-trip integration test (ZEB-110)"
```

---

### Task 5: Manifest integrity, crash simulation, and deterministic CID tests
...
- [ ] **Step 2: Write the crash simulation test**
...
- [ ] **Step 3: Write the deterministic CID test**
...
Run: `cargo test -p harmony-oluo persist`
...
```bash
git add crates/harmony-oluo/src/persist.rs
git commit -m "test(oluo): manifest integrity, crash simulation, deterministic CID tests (ZEB-110)"
```
...
- [ ] **Step 1: Run the full harmony-oluo test suite**

Run: `cargo test -p harmony-oluo`
Expected: all tests pass (existing engine tests + new persist tests)

- [ ] **Step 2: Run workspace check to verify no dependency issues**
...
Only commit if steps 1-2 revealed issues that required fixes.
